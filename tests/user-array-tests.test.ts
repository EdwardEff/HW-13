...
test.describe("User management API", () => {
  ...
  test("14.2 Проверка удаления всех юзеров", async ({ request }) => {
    ...
    const responseUsersAfterDeletion = await request.get(
      `${baseURLWithEndpoint}`,
    );
    const responseUsersAfterDeletionJson: UserDTO[] =
      await responseUsersAfterDeletion.json();
  ...
  test("14.3 Проверка удаления одного из юзеров", async ({ request }) => {
    ...
    const responseUsersAfterDeletionJson: UserDTO[] =
      await responseUsersAfterDeletion.json(); // 4

    expect(responseBody.length).toBeGreaterThan(
      responseUsersAfterDeletionJson.length,
    );
  });
});
